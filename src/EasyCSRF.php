...
class EasyCSRF
{
   ...
   public function check($key, $token, $timespan = null, $multiple = false)
   {
      ...
      if (sha1($_SERVER['REMOTE_ADDR'] . $_SERVER['HTTP_USER_AGENT']) != substr(base64_decode($session_token), 10, 40)) return false;

      if ($token != $session_token) return false;

      // Check for token expiration
      if ($timespan != null && is_int($timespan) && intval(substr(base64_decode($session_token), 0, 10)) + $timespan < time()) return false;
